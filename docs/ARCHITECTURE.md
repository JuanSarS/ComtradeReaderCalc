...
- `Src/ui/desktop/`: native PyQt6 shell, local web host lifecycle, native file dialogs
- `Src/ui/web/`: embedded Dash application factory, local REST endpoints, shared state
- `Src/core/analysis/`: shared engineering modules and orchestration pipeline
- `Src/ui/pages/`: existing Dash pages/charts/layout retained as compatibility view layer
- `(legacy)`: legacy PyQt tab UI retained temporarily and no longer used by the main entry point
...
2. `Src/ui/desktop/server.py` starts a local Dash server on `127.0.0.1` in a background thread.
3. `Src/ui/desktop/main_window.py` loads that URL in `QWebEngineView`.
4. `Open COMTRADE` uses a native desktop file dialog.
5. `Src/core/analysis/analysis_pipeline.py` processes the selected COMTRADE pair.
6. `Src/ui/web/state.py` shares the resulting analysis with Dash callbacks.
...
- `(legacy)main_window.py`: legacy PyQt tab shell, no longer used by the main entry point
- `(legacy)tabs/*`: legacy desktop tabs replaced by the embedded web UI
- `Src/ui/pages/`: compatibility rendering layer; can be migrated gradually into `Src/ui/web/`
...
- Required data inclusion: `Src/ui/pages/assets`, plus any future static resources
